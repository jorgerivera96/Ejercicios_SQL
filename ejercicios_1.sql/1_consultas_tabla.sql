...
SELECT 
    ...
    ROUND((precio * 1.1)::numeric, 2) AS precio_usd
FROM producto;
...
SELECT 
    LOWER(nombre) AS "nombre de producto", 
    precio AS euros
FROM producto;
...
SELECT 
    nombre,
    UPPER(SUBSTRING(nombre, 1, 2)) AS dos_iniciales
FROM fabricante;
...
/* 36. Lista el nombre y el precio de todos los productos que tengan un precio mayor o igual 
a 180€. Ordene el resultado en primer lugar por el precio (en orden descendente) y en segundo 
